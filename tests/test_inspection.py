...
def test_get_coef():
    ...
    full_pipe = Pipeline([("scaler", tml.DfScaler()), ("logit", LogisticRegression(solver="lbfgs"))])
...
@pytest.mark.parametrize(
    "model", [DecisionTreeClassifier(), XGBClassifier(n_estimators=10), LGBMClassifier(n_estimators=10)]
)
def test_feat_imp(model):
    ...
    full_pipe = Pipeline([("scaler", tml.DfScaler()), ("model", model)])
...
def test_learning_curves_xgb(_):
    ...
    full_pipe = Pipeline(
        [("scaler", tml.DfScaler()), ("xgb", XGBClassifier(objective="binary:logistic", n_estimators=2, n_jobs=-1))]
    )
...
def test_plot_feat_imp_warning():
    ...
    """
    wrong_input = pd.DataFrame({"a": [1, 2, 3], "b": [2, 3, 4]})
...
def test_plot_pdp(_):
    ...
    full_pipe = Pipeline([("imputer", tml.DfImputer()), ("lgb", LGBMClassifier(n_estimators=10))])
