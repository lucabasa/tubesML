...
def test_stacker_cls():
    ...
    estm = [("tree1", DecisionTreeClassifier(max_depth=3)), ("tree2", DecisionTreeClassifier(max_depth=5))]
    ...
    with warnings.catch_warnings():
        ...
        with warnings.catch_warnings():
            ...
            stk = tubesml.Stacker(estimators=estm, final_estimator=LogisticRegression(), cv=kfold)
            stk.fit(df_1, y)
            _ = stk.predict(df_1)
...
def test_stacker_reg():
    ...
    with warnings.catch_warnings():
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            stk = tubesml.Stacker(estimators=estm, final_estimator=DecisionTreeRegressor(), cv=kfold)
            stk.fit(df_1, y)
            _ = stk.predict(df_1)
...
def test_importances(passthrough):
    ...
    estm = [("tree", DecisionTreeClassifier(max_depth=3)), ("logit", LogisticRegression())]

    kfold = KFold(n_splits=3)
    stk = tubesml.Stacker(estimators=estm, final_estimator=DecisionTreeClassifier(), cv=kfold, passthrough=passthrough)
    stk.fit(df_1, y)
...
def test_early_stopping():
    ...
    estm = [
        ("xgb", XGBClassifier(n_estimators=10000, early_stopping_rounds=5, eval_metric="logloss")),
...
def test_early_stopping_pipeline_estimators():
    ...
    estm = [
        (
            ...
            Pipeline(
                [
                    ("scl", tubesml.DfScaler()),
                    ("xgb", XGBClassifier(n_estimators=10000, early_stopping_rounds=5, eval_metric="logloss")),
                ]
            ),
        ),
        ("lgb", Pipeline([("scl", tubesml.DfScaler()), ("lgb", LGBMClassifier(n_estimators=10000))])),
    ]
...
def test_high_correlation_warning():
    ...
    We use the same model twice so we are sure the predictions are correlated.
    """
    y = df["target"]
    df_1 = df.drop("target", axis=1)
    ...
    with pytest.warns(UserWarning):
        stk = tubesml.Stacker(estimators=estm, final_estimator=DecisionTreeClassifier(), cv=kfold, verbose=True)
        stk.fit(df_1, y)
...
def test_gridsearch_stacker_simple(scoring):
    ...
    with warnings.catch_warnings():
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            result, best_param, best_estimator = tubesml.grid_search(
                data=df_1, target=y, estimator=stk, param_grid=param_grid, scoring=scoring, cv=3
            )
...
def test_gridsearch_stacker_pipeline(scoring):
    ...
    with warnings.catch_warnings():
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            result, best_param, best_estimator = tubesml.grid_search(
                data=df_1, target=y, estimator=pipe, param_grid=param_grid, scoring=scoring, cv=3
            )
...
def test_gridsearch_stacker_passthrough(passthrough):
    ...
    with warnings.catch_warnings():
        ...
        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", category=DeprecationWarning)
            result, best_param, best_estimator = tubesml.grid_search(
                data=df_1, target=y, estimator=pipe, param_grid=param_grid, scoring="neg_log_loss", cv=3
            )
