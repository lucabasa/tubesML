...
def plot_learning_curve(estimator, X, y, scoring=None, ylim=None, cv=None, n_jobs=None, train_sizes=None, title=None):
    """
    ...
    train_sizes, train_scores, test_scores, fit_times, score_times = learning_curve(
        estimator, X, y, cv=cv, n_jobs=n_jobs, scoring=scoring, train_sizes=train_sizes, return_times=True
    )
    ...
    ax[0][0].plot(train_sizes, train_scores_mean, "o-", color="r", label="Training score")
    ax[0][0].plot(train_sizes, test_scores_mean, "o-", color="g", label="Cross-validation score")
    ...
    ax[0][1].fill_between(train_sizes, fit_times_mean - fit_times_std, fit_times_mean + fit_times_std, alpha=0.1)
    ...
    ax[1][0].fill_between(
        fit_times_mean, test_scores_mean - test_scores_std, test_scores_mean + test_scores_std, alpha=0.1
    )
    ...
    ax[1][1].fill_between(
        fit_times_mean, train_scores_mean - train_scores_std, train_scores_mean + train_scores_std, alpha=0.1
    )
...
def get_pdp(estimator, feature, data, grid_resolution=100):
    ...
    if isinstance(feature, tuple):
        ...
        raise TypeError(
            "This function does not support the calculation over multiple features."
            + "You can use directly the sklearn function for that."
        )
    ...
    if isinstance(feature, tuple):
        ...
        df_pdp = pd.DataFrame({"x": pdp[vals][0], "x_1": [np.nan] * len(pdp[vals][0]), "y": pdp["average"][0]})
...
def plot_two_pdp(data, feature, title, axes):
    ...
    xg, yg = np.meshgrid(np.linspace(X_axis.min(), X_axis.max(), 100), np.linspace(Y_axis.min(), Y_axis.max(), 100))
    ...
    axes.contourf(
        xg, yg, zg, norm=plt.Normalize(vmax=plt_data["y"].max(), vmin=plt_data["y"].min()), cmap=plt.cm.terrain
    )

    axes.set_xlabel(feature[0], fontsize=12)
    axes.set_ylabel(feature[1], fontsize=12)
    axes.set_title(title, fontsize=14)
