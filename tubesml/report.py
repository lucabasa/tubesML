...
def _plot_diagonal(ax):
    ...
    line = pd.DataFrame(
        {"x": np.arange(low, high, scl), "y": np.arange(low, high, scl)}  # small hack for a diagonal line
    )
...
def _plot_simple_predictions(fig, ax, tmp, hue):
    ...
    sns.scatterplot(x="True Label", y="Prediction", data=tmp, ax=ax[0], hue=hue, legend=legend, alpha=0.5)
...
def _plot_feature_predictions(fig, ax, tmp, feature):
    ...
    sns.scatterplot(x=feature, y="Prediction", data=tmp, ax=ax[0], label="Prediction", legend="full", alpha=0.7)

    sns.scatterplot(x=feature, y="Residual", data=tmp, ax=ax[1], legend=legend, alpha=0.7)
...
def plot_confusion_matrix(true_label, pred_label, ax=None, thrs=0.5):
    ...
    sns.heatmap(cm, cbar=False, ax=ax, annot=True, fmt=".2%", annot_kws={"size": 15}, linewidths=0.5, cmap="coolwarm")
...
def plot_classification_probs(
    ...
    if hue_feat is None:
        tmp[["True Label", "Prediction"]].mean().plot(kind="bar", ax=ax[1][1], color=["r", "g"], alpha=0.7)
    else:
        tmp.groupby(hue_feat)[["True Label", "Prediction"]].mean().plot(
            kind="bar", ax=ax[1][1], color=["r", "g"], alpha=0.7
        )
