...
class Stacker(BaseTransformer, RegressorMixin, ClassifierMixin):
    ...
    full dataset. The predictions are generated by ``tubesml.CrossValidate`` and thus can
    account for early stopping and other options.

    The meta estimator trains on the predictions made by the estimators. It is possible to
    train the meta estimator on additional features from the original dataset.

    :param estimators: list of tuples ``(name, estimator)``.
        These estimators generate the first layer of predictions.

    :param final_estimator: estimator or str.
        Estimator with ``fit`` and ``predict`` (or ``predict_proba``) methods.
        If set to ``"blend"``, no final estimator is fitted and the level‑1 predictions
        are simply averaged.

    :param cv: KFold-like generator.
        Cross-validation scheme used to generate the first layer of predictions.

    :param lay1_kwargs: dict, optional.
        Dictionary of settings passed to ``tubesml.CrossValidate`` for the first layer.
        Keys must match the names in ``estimators``.

    :param passthrough: bool or list, default False.
        If True, all features used in the first layer are passed to the final estimator.
        If a list, only those features are passed. Invalid input defaults to False.

    :param verbose: bool, default False.
        If True, warns the user when the correlation between first-layer predictions
        exceeds 0.9.

    :Attributes:

        ``meta_importances_`` : pandas.DataFrame
            Feature importances (or coefficients) of the final estimator. If the final
            estimator does not expose ``coef_`` or ``feature_importances_``, this
            attribute will not be available.

        ``corr_`` : pandas.DataFrame
            Correlation matrix of the first-layer predictions.
    ...
    def __init__(
        ...
        if not type(self.passthrough) in [list, bool]:
            warnings.warn(
                f"passthrough must be a list or a boolean, we got {type(self.passthrough)} it will be ignored",
                UserWarning,
            )
    ...
    def fit(self, X, y):
        """
        This method uses ``tubesml.cv_score`` to create out of fold predictions from each of the estimators provided
        in ``estimators``.

        Secondly, it fits the `final_estimator` on a dataset that contains these predictions and any feature specified
        by ``passthrough``.
        Each of the ``estimators`` is then refit on the entire dataset

        If an estimator is in the first layer of estimators was trained with early stopping, in the refit it will be
        trained on a number of iterations equal to the mean number across the folds used to generate the first layer
        of predictions. Be sure that the early stopping attribute of the estimator is ``early_stopping_round``.

        If ``verbose=True`` the user will be warned if any of the predictions are correlated more than 0.9.
        The ``corr_`` attribute is created by this method
        ...
        for i, est in enumerate(self._estimators):
            ...
            if self.lay1_kwargs[self.estimators[i][0]]["early_stopping"]:
                try:
                    self._estimators[i].set_params(
                        **{"n_estimators": np.mean(res["iterations"]).astype(int), "early_stopping_rounds": None}
                    )
                except ValueError:  # if the estimator is a pipeline, setting n_estimators requires a workaround
                    self._estimators[i].steps[-1][1].set_params(
                        **{"n_estimators": np.mean(res["iterations"]).astype(int), "early_stopping_rounds": None}
                    )
    ...
    def _make_predict_test(self, X):
        ...
        for i, _ in enumerate(self._estimators):
