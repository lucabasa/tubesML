...
class Stacker(BaseTransformer):
    ...
    on the entire dataset via a KFold and it is then re-fitted against the full dataset. The predictions are
    generated by ``tubesml.CrossValidate`` and thus can account for early stopping and other options.
    ...
    :param lay1_kwargs: (optional) dict,
            Dictionary of settings for the first layer predictions to be passed to ``tubesml.CrossValidate``.
            The keys of the dictionary refer to the names in the list of ``estimators``.

    :param passthrough: bool or list, default=False.
                        If True, all the features used to train the first layer of estimators will be used
    ...
    :Attributes:
        ...
        `meta_importances_` : pandas DataFrame with the feature importances (or the coefficients) of the
            final estimator. Note: this estimator doesn't have a ``coef_`` or ``feature_importances_`` attribute,
